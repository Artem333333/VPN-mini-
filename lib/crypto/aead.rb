  ...
  module Crypto
    class AEAD
      ...
      def validate_key!(key)
        raise HOVPN::Core::Errors::CryptoError, "Key must be #{KEY_SIZE} bytes" unless key&.bytesize == KEY_SIZE
      end

      def validate_nonce!(nonce)
        raise HOVPN::Core::Errors::CryptoError, "Nonce must be #{NONCE_SIZE} bytes" unless nonce&.bytesize == NONCE_SIZE
      end

      def prevent_nonce_reuse!(nonce)
        raise HOVPN::Core::Errors::CryptoError, "CRITICAL: Nonce reuse detected!" if @last_nonce == nonce
        @last_nonce = nonce
      end
    end
  end
