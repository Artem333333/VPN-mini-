  ...
  module Crypto
    class KeyPair
      ...
      def shared_secret(remote_public_hex)
        ...
        shared = RbNaCl::GroupElements::Curve25519.new(remote_raw)
      ...
      def save_encrypted!(path, password)
        ...
        File.open(path, 'w', 0600) { |f| f.write(payload.to_json) }
      ensure
        wipe_string!(cipher_key) if defined?(cipher_key)
      end

      def private_hex; @private_key.to_bytes.unpack1('H*'); end
      def public_hex;  @public_key.to_bytes.unpack1('H*');  end
      ...
      def validate_key_size!(raw_bytes)
        unless raw_bytes.bytesize == KEY_SIZE
